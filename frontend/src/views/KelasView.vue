<template>
  <div>
    ...
    <el-card shadow="hover" class="mb-4">
      <template #header>
        <div class="flex justify-between items-center">
          <span>Filter</span>
          <el-button type="primary" size="small" @click="openDialog()">+ Tambah Kelas</el-button>
        </div>
      </template>
      <el-row :gutter="16">
        <el-col :span="8">
    ...
    <el-card shadow="hover">
      <el-table :data="kelasStore.list" v-loading="kelasStore.loading" stripe style="width: 100%">
        <el-table-column prop="nama" label="Nama Kelas" />
        <el-table-column prop="tingkat" label="Tingkat" width="120" />
        <el-table-column prop="wali_kelas" label="Wali Kelas" min-width="150" />
        <el-table-column label="Tahun Ajaran" min-width="200">
          <template #default="{ row }">
            {{ row.tahun_ajaran?.nama }} - {{ row.tahun_ajaran?.semester }}
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="140px">
        <el-form-item label="Nama Kelas" prop="nama">
          <el-input v-model="form.nama" placeholder="Contoh: XII IPA 1" />
        </el-form-item>
        ...
        <el-form-item label="Wali Kelas" prop="wali_kelas">
          <el-input v-model="form.wali_kelas" placeholder="Nama wali kelas" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const rules: FormRules = {
  nama: [{ required: true, message: 'Nama kelas harus diisi', trigger: 'blur' }],
  tingkat: [{ required: true, message: 'Pilih tingkat', trigger: 'change' }],
  tahun_ajaran_id: [{ required: true, message: 'Pilih tahun ajaran', trigger: 'change', type: 'number' }],
  wali_kelas: [{ required: true, message: 'Nama wali kelas harus diisi', trigger: 'blur' }],
}
...
function openDialog(row?: KelasForm & { id?: number }) {
  if (row) {
    ...
    form.nama = ''
    form.tingkat = '1'
    form.tahun_ajaran_id = tahunAjaranStore.list.find((t) => t.is_active)?.id || 0
    form.wali_kelas = ''
  }
...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm('Yakin ingin menghapus kelas ini?', 'Konfirmasi', {
      type: 'warning',
    })
