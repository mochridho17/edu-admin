<template>
  <div>
    <el-card shadow="hover">
      <template #header>
        <div class="flex justify-between items-center">
          ...
          <el-button type="primary" size="small" @click="openDialog()">+ Tambah User</el-button>
        </div>
      </template>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? 'Edit User' : 'Tambah User'" width="500px">
...
function openDialog(row?: User) {
  if (row) {
    ...
    form.username = row.username; form.email = row.email; form.password = ''; form.role = row.role
    // Password tidak wajib saat edit
    rules.password = []
  } else {
    isEdit.value = false; editingId.value = null
    form.username = ''; form.email = ''; form.password = ''; form.role = 'admin'
    rules.password = [{ required: true, message: 'Password harus diisi', trigger: 'blur' }]
  }
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value && editingId.value) {
      const payload: Partial<UserForm> = { username: form.username, email: form.email, role: form.role }
...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm('Yakin ingin menghapus user ini?', 'Konfirmasi', { type: 'warning' })
