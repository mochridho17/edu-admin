  ...
  <div>
    ...
    <el-card shadow="hover" class="mb-4">
      <el-row :gutter="16">
        ...
        <el-col :span="6">
          <el-button type="primary" :loading="absensiStore.loading" @click="loadAbsensi">
    ...
    <el-card shadow="hover" v-if="selectedKelas && selectedTanggal">
      <div class="mb-4 flex justify-between items-center">
        <span class="text-gray-600">
          <strong>{{ selectedKelasNama }}</strong> — {{ formatTanggal(selectedTanggal) }}
        </span>
      ...
      <el-table :data="tableData" stripe style="width: 100%">
        ...
        <el-table-column prop="nama" label="Nama Siswa" min-width="200" />
        <el-table-column label="Status Kehadiran" width="400">
          <template #default="{ row }">
            <el-radio-group v-model="row.status" class="status-group">
              <el-radio-button value="hadir">Hadir</el-radio-button>
              <el-radio-button value="sakit">Sakit</el-radio-button>
        ...
        <el-table-column label="Keterangan" width="200">
          <template #default="{ row }">
            <el-input v-model="row.keterangan" placeholder="Opsional" size="small" />
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <!-- Empty State -->
    <el-empty v-else description="Pilih kelas dan tanggal untuk memulai absensi" />
  </div>
</template>
...
async function loadAbsensi() {
  ...
  try {
    await absensiStore.fetchByKelasTanggal(
      selectedKelas.value, selectedTanggal.value
    )
    if (absensiStore.list.length > 0) {
      ...
      const resSiswa = await siswaApi.getAll({ kelas_id: selectedKelas.value })
