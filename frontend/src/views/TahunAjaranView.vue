<template>
  <div>
    <el-card shadow="hover">
      <template #header>
        <div class="flex justify-between items-center">
          ...
          <el-button type="primary" size="small" @click="openDialog()">+ Tambah Tahun Ajaran</el-button>
        </div>
      </template>
      <el-table :data="tahunAjaranStore.list" v-loading="tahunAjaranStore.loading" stripe style="width: 100%">
        <el-table-column prop="nama" label="Tahun Ajaran" width="150" />
        <el-table-column prop="semester" label="Semester" width="100">
          <template #default="{ row }">
            <el-tag :type="row.semester === 'ganjil' ? 'primary' : 'warning'">
        ...
        <el-table-column label="Status" width="120">
          <template #default="{ row }">
            <el-tag v-if="row.is_active" type="success">Aktif</el-tag>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? 'Edit Tahun Ajaran' : 'Tambah Tahun Ajaran'" width="500px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="140px">
        <el-form-item label="Tahun Ajaran" prop="nama">
          <el-input v-model="form.nama" placeholder="Contoh: 2025/2026" />
        </el-form-item>
...
const rules: FormRules = {
  nama: [{ required: true, message: 'Tahun ajaran harus diisi', trigger: 'blur' }],
  semester: [{ required: true, message: 'Pilih semester', trigger: 'change' }],
}

onMounted(() => tahunAjaranStore.fetchAll())

function openDialog(row?: TahunAjaranForm & { id?: number }) {
  if (row) {
    isEdit.value = true; editingId.value = row.id!
    form.nama = row.nama; form.semester = row.semester
  } else {
    isEdit.value = false; editingId.value = null
    form.nama = ''; form.semester = 'ganjil'
  }
...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm('Yakin ingin menghapus tahun ajaran ini?', 'Konfirmasi', { type: 'warning' })
